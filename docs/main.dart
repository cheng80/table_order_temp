...
// ---------------------------------------------------------------------------
// [Viewer] 13개 화면 통합 뷰어 (좌측 리스트 - 우측 프리뷰)
// ---------------------------------------------------------------------------
...
class _TabletMainViewerState extends State<TabletMainViewer> {
  ...
  final List<Widget> _screens = [
    ...
    const ScreenO02MenuMasterDetail(), // O-02 + O-03 통합
    const ScreenO04Banner(),
    const ScreenO05Table(),
    const ScreenO06Inquiry(),
    const ScreenT01ScrollSpy(), // ★ 확정된 스크롤스파이 메뉴
    const ScreenT02Option(),
    const ScreenT03Cart(), // ★ 확정된 분할 결제
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF1E1E1E), // 개발자 도구 느낌 배경
      body: SafeArea(
        child: Row(
          children: [
            // [Left] Screen List
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              ...
              height: 56,
              child: FilledButton(onPressed: () {}, child: const Text("가입하기")),
            ),
          ],
        ),
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              width: double.infinity,
              height: 56,
              child: FilledButton(onPressed: () {}, child: const Text("로그인")),
            ),
            const SizedBox(height: 16),
            TextButton(
              onPressed: () {},
...
class ScreenO02MenuMasterDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("메뉴 통합 관리 (Split View)"), elevation: 1),
      body: Row(
        children: [
          // List Area
          Expanded(
...
// ===========================================================================
// [Group T] 테이블 (Table) - ScrollSpy & 결제 기능 고도화
// ===========================================================================
...
class _ScreenT01ScrollSpyState extends State<ScreenT01ScrollSpy> {
  ...
  void _calculateOffsets() {
    ...
    for (int i = 0; i < _categories.length; i++) {
      ...
      // Rows calculation (crossAxisCount: 3)
      int rows = (count / 3).ceil();
      // Header(60) + Items(240*rows) + Spacing(16*rows)
          ...
          SizedBox(
            ...
            child: NavigationRail(
              ...
              destinations: _categories.map((cat) {
                ...
                else if (cat.contains("파스타"))
                  icon = Icons.lunch_dining;
                else if (cat.contains("피자"))
                  icon = Icons.local_pizza;
                else if (cat.contains("샐러드"))
                  icon = Icons.grass;
                else if (cat.contains("음료"))
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    RadioListTile(
                      ...
                      groupValue: 1,
                      onChanged: (v) {},
                      title: const Text("매운맛"),
                    ),
                    RadioListTile(
                      value: 2,
                      groupValue: 1,
                      onChanged: (v) {},
                      title: const Text("순한맛"),
                    ),
                    const Spacer(),
                    SizedBox(
                      width: double.infinity,
...
class _ScreenT03CartState extends State<ScreenT03Cart> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var item in _items)
      grandTotal += (item["price"] as int) * (item["qty"] as int);
    ...
    if (_paymentMode == 0) {
      ...
      for (int i in _selectedIndices)
        selectedTotal +=
            (_items[i]["price"] as int) * (_items[i]["qty"] as int);
    }
                  ...
                  Expanded(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        bool isSelected = _paymentMode == 0
                            ? true
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text("총 주문 금액", style: TextStyle(color: Colors.grey)),
                    Text(
                      "$grandTotal원",
                      style: const TextStyle(
                        color: Colors.grey,
