네, **Mermaid 차트의 작동 방식**에 대해 먼저 명확히 답변드리고, 요청하신 **상세 비즈니스 로직 문서**를 작성해 드리겠습니다.

### 1. Mermaid 차트의 특징 (사용자님의 의문점)

> "이동이나 속성을 바꾼다거나 이런 건 없고 방향 설정 정도만 있던데 이게 맞아?"

**네, 정확하게 보셨습니다.**
Mermaid는 파워포인트나 피그마(Figma)처럼 마우스로 도형을 드래그해서 예쁘게 배치하는 **"그리기 도구"**가 아니라, **"코드로 구조를 정의하면(Layout Engine) 알아서 그려주는 도구"**입니다.

* **장점:** 내용(로직)에만 집중하면 디자인은 자동으로 통일됩니다. 깃허브(GitHub) 등에서 코드로 형상 관리가 가능합니다.
* **단점:** "이 화살표를 조금만 오른쪽으로..." 같은 미세한 위치 조정이 불가능합니다.
* **제어 가능 항목:**
* 방향: `TD`(위→아래), `LR`(왼쪽→오른쪽)
* 그룹: `subgraph`로 묶기
* 순서: 코드에 적힌 순서대로 배치됨
...
### 2. [개정판] 상세 비즈니스 로직 및 워크플로우 명세서

기존 04번 문서를 완전히 대체하는 **Deep-Dive(심화) 버전**입니다.
실제 개발 시 발생할 수 있는 **유효성 검사, API 통신 실패, 예외 처리, UI 피드백**을 모두 포함하여 "진짜 사용되는 로직"으로 구현했습니다.

이 내용을 복사하여 **`04_business_logic_workflow_detail.md`** 파일로 저장하십시오.
